...
object Interaction2 extends Interaction2Interface {
  ...
  def availableLayers: Seq[Layer] = {
    Seq(
      Layer(LayerName.Temperatures, tempTable, 1990 to 1991),
      Layer(LayerName.Deviations, tempDevTable, 1990 to 1991)
    )
  }
  ...
  def yearBounds(selectedLayer: Signal[Layer]): Signal[Range] = {
    return Signal(selectedLayer().bounds)
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    ...
    Signal(boundsSignal().find(_ == sliderValue()).getOrElse(valOrExtrem(sliderValue(), boundsSignal().min, boundsSignal().max)))
  }

  def valOrExtrem(value: Int, min: Int, max: Int): Int = value.min(max).max(min)
